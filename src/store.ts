...
export const DEFAULT_QUERY_MODEL = "hf:Qwen/Qwen2.5-1.5B-Instruct-GGUF/qwen2.5-1.5b-instruct-q4_k_m.gguf";
  ...
  while (i < s.length) {
    ...
    if (s[i] === '"') {
      ...
      for (const t of terms) {
        if (isHyphenatedToken(t)) {
          const sanitized = sanitizeHyphenatedTerm(t);
          if (sanitized) {
