...
QMD-Chinese 更适合运行在 **本地 PC 工作站**（Windows WSL2 或原生 Linux），不太建议放在低配 VPS（如 1核1G、2核4G）上长期跑，多模型并发时容易出现内存吃紧甚至被 OOM 杀掉。

**最低建议配置（可轻量使用）：**
* **CPU：** 4 核
* **内存：** 8 GB
* **适用场景：** 个人本地知识库、小到中等规模文档检索、轻量级查询与验证

**更舒服的体验建议：**
* **本地 PC 工作站：** 8 核 CPU + 16 GB 内存或更高
* **显卡（可选，但推荐）：** 如果配有 **NVIDIA 独立显卡**（如 RTX 30 / 40 / 50 系），并正确启用 CUDA，加速效果会明显更好，索引、检索和生成响应都会快很多。
* **没有独显也能跑：** 纯 CPU 环境也可以使用，只是首次加载和复杂查询会更慢，更适合轻量场景。
* **网络需求：** 首次下载模型需要联网。请注意，**普通的 proxy 环境变量对 Node.js 原生 fetch 不一定生效**。在国内网络环境下，建议通过设置 Hugging Face 国内镜像来完成下载。
